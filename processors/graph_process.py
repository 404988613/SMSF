...
class GraphSteganalysisProcessor(object):
    ...
    def get_dev_examples(self, dir):
        return self.get_examples(os.path.join(dir, "dev.csv"))
...
def preprocess_data(corpus, stego_method, dataset, tokenizer_type,):
    ...
    for datatype in ["train", "dev", "test"]:
        ...
        examples = get_examples(data_home)
        results = processor.convert_examples_to_features(examples, )
        all_input_ids, all_input_mask, all_token_type_ids, all_label_ids, all_dependency_matrix = results.tensors

        processed_data = {}
        processed_data["tokenized_sentence"] = all_input_ids  # tensor:[len,128]
        processed_data["tokenized_sentence_attn_mask"] = all_input_mask  # tensor:[len,128]
        processed_data["token_type_ids"] = all_token_type_ids  # tensor:[len,128]
        processed_data["label"] = final_label  # list:[len]
        processed_data["sentence"] = final_sentence  # list:[len]
        processed_data["sentence_dependency_matrix"] = all_dependency_matrix  # tensor:[len,128,128]
