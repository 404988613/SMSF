...
class SeqInputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, input):
...
class SteganalysisProcessor(object):
    ...
    def _create_examples(self, file_name):
        ...
        for i, line in enumerate(lines):
            if i > 0:
                ...
                s1_split = s1.split()
                s2 = ""
                for k in range(0, len(s1_split) - 1):
                    s2 += s1_split[k]
                    if s1_split[k + 1] == "n't":
                        continue
                    else:
                        s2 += " "
                s2 += s1_split[len(s1_split) - 1]
                # 将缩写自动替换为展开形式
                sentence = contractions.fix(s2)
    ...
    def convert_examples_to_features(self, examples):
        ...
        all_input = [f.input for f in features]
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)
        return dataset, all_input
        # return features
