...
class TC(nn.Module):
    def __init__(self,  vocab_size=30522, embed_dim=256, class_num=2, num_layers=1, hidden_dim=200, dropout_rate=0.2):
    ...
    def forward(self, inputs=None, tokenized_inputs=None, attn_mask=None, model_type=None):
        embedding = self.embed(tokenized_inputs)
        logits, out = self.classifier(embedding)
